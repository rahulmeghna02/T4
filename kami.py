...
def approve_or_disapprove_user(message):
    ...
    if len(cmd_parts) < 2:
        bot.send_message(
            ...
            "*User {target_user_id} ye Madharchod ko approve dediya *\n"
            reply_markup=create_inline_keyboard(), parse_mode='Markdown')
        return
    ...
    try:
        ...
        bot.send_message(chat_id, "*💣 Taiyar Ho Gaya lobby ki gand Marne Ke Liye?*\n"  # Ready to launch message
                                   "*Chal madharchod jaldi se yah provide kar mujhe the target IP, port, and duration in seconds.*\n"  # Provide details message
    ...
    try:
        ...
        if len(args) != 3:
            bot.send_message(chat_id, "*❗ Invalid format!*\n"
                                       "*Sahi format use karna chutiye and try again.*\nn"
        ...
        sent_message = bot.send_message(chat_id, f"*🚀 Attack Initiated! 🚀*\n\n"
                                                 ...
                                                 f"*⏰ Duration: {duration} seconds! Gand marne mai maza aarha hai ! 🔥*", 
                                                 "*Prepare for action! 🔥*", 
                                                 reply_markup=create_inline_keyboard(), parse_mode='Markdown')
        ...
        for remaining_time in range(duration, 0, -1):
            ...
            new_message_text = (f"*🚀 Attack Initiated! 🚀*\n\n"
                                f"*📡 Target Host: {target_ip}*\n"
                                f"*👉 Target Port: {target_port}*\n"
                                f"*⏰ ⏰ Duration: {duration} seconds! Gand marne mai maza aarha hai ! 🔥*"n"
                                "*Prepare for action! 🔥*")
...
def myinfo_command(message):
    try:
        ...
        if not user_data:
            response = (
                "*❌ Oops! No account information found!* \n"
                "*🔒 Your account is still pending approval!* \n"
                "*Koi bhi problem ho to baap se baat kar: @RAHUL_DDOS_B * 🙏"
            )
            markup = types.InlineKeyboardMarkup()
